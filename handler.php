...
if(!empty($support_array['results']) && !empty($support_array['agents'])) {
  if ($option_format === 'js' || $option_format === 'json' || $option_callback) {
    ...
    if ($option_format === 'html' || $option_html) {
      $support_array['html'] = html_encode($support_array, $option_style, !$option_nocss, $option_template);
    }
    ...
    $string = preg_replace('/<%= content %>/', html_encode($support_array, $option_style, !$option_nocss, $option_template), $string);
